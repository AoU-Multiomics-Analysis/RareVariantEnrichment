...
HEADER = 'ome_name\tphenotype_bed\tprincipal_components_tsv\tadditional_covariates_tsv\n'
...
def test_manifest_keeps_paths_as_metadata_and_optional_fields(tmp_path):
    result = prepare(tmp_path, HEADER +
        'protein\tgs://bucket/protein.bed.gz\tgs://bucket/protein.pcs.tsv\tgs://bucket/age.tsv\n'
        'splicing\tgs://bucket/splicing.bed.gz\tgs://bucket/splicing.pcs.tsv\t\n')
...
def test_manifest_allows_optional_columns_to_be_omitted_and_headers_reordered(tmp_path):
    result = prepare(tmp_path, 'principal_components_tsv\tome_name\tphenotype_bed\n'
        'gs://bucket/a.pc\ta\tgs://bucket/a.bed\n'
        'gs://bucket/b.pc\tb\tgs://bucket/b.bed\n')
    assert result.returncode == 0, result.stderr
    assert (tmp_path / 'normalized.tsv').read_text().splitlines()[0] == 'a\tgs://bucket/a.bed\tgs://bucket/a.pc\t.'
...
@pytest.mark.parametrize('text,error', [
    ...
    (HEADER + 'a\trelative.bed\tgs://bucket/pc\t.\n', 'absolute'),
    (HEADER + 'a\tgs://bucket\tgs://bucket/pc\t.\n', 'object'),
    (HEADER + 'a\tgs://bucket/a\t\t.\n', 'principal_components_tsv'),
    (HEADER + 'a\tgs://bucket/a\tgs://bucket/pc\n', 'columns'),
    (HEADER + 'a\tgs://bucket/a\tgs://bucket/pc\t.\n', 'two'),
])
...
def test_manifest_preserves_quotes_for_wdl_read_tsv(tmp_path):
    manifest = tmp_path / 'manifest.tsv'
    rows = [
        ['rna', '/data/a "quoted".bed', '/data/pc.tsv', '.'],
        ['protein', "/data/b 'quoted'.bed", '/data/pc.tsv', '.'],
    ]
